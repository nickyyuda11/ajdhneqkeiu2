...
function login($username, $password, $proxy = 0, $proxyauth = 0)
{
    ...
    if ($proxy) {
        $get = curl($url_ig, 1, 1, $username, $proxy, $proxyauth);
    } else {
        $get = curl($url_ig);
    }
...
function ReplyComment($id, $id_comment, $session, $text, $proxy = 0, $proxyauth = 0)
{
    $url = 'https://www.instagram.com/web/comments/' . $id . '/add/';
    $header = array(
        'User-Agent: Mozilla/5.0 (iPhone; CPU iPhone OS 11_0_1 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A402 Safari/604.1',
    ...
    $body = 'comment_text=' . $text . '&replied_to_comment_id=' . $id_comment . '';
    if ($proxy) {
        $post = curlNoHeader($url, $body, $header, $session['username'], $proxy, $proxyauth);
    } else {
        $post = curlNoHeader($url, $body, $header);
...
function KirimDM($message, $thread_ids, $session)
{
    ...
    $body = '{"text":"' . $message . '","thread_ids":"["' . $thread_ids . '"]"}';
    $body = urlencode('ig_sig_key_version=4&signed_body=7aa4d66e99c7c85519159a9ea6bcdba5d8fd4218971d6896a166b08bc1c04f7f.' . $body . '');
    echo $body;
...
function ProcessDM($userids, $session)
{
    ...
    $body = urlencode('signed_body=e78502cd55e0329babd8200a01354490407d5b8f7f4ff25f2979d3049ff4c28e&ig_sig_key_version=4.' . $body . '');
    echo $body;
    $post = curl($url, $body, $header);
    var_dump($post);
    $response = json_decode($post['body'], true);
...
{
    if (isset($shortcode)) {
        $url_media = 'https://www.instagram.com/graphql/query/?query_hash=d5d763b1e2acf209d62d22d184488e57&variables={"shortcode":"{{shortcode}}","include_reel":true,"first":"{{count}}","after":"{{after}}"}';
        $url = str_replace('{{shortcode}}', $shortcode, $url_media);
...
function getMediaLikers($username, $session, $count = 20, $pageSize = 20, $proxy = 0, $proxyauth = 0)
{
    ...
    while (true) {
        ...
        $url = getMediaLink($username, 20, $endCursor);
...
function getFollowers($username, $session, $count = 20, $pageSize = 20, $proxy = 0, $proxyauth = 0)
{
    ...
    while (true) {
        if ($proxy) {
            $url = getFollowersLink($username, 20, $endCursor, $session, $proxy, $proxyauth);
        } else {
