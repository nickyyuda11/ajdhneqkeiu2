...
$login = login($u, $p, $proxy, $proxyauth);
if ($login['status'] == 'success') {
    ...
    $data_target = findProfile($username_target, $data_login, $proxy, $proxyauth);
    ...
    if ($data_target['status'] == 'success') {
        ...
        for ($i = 1; $i < $data_target['followers']; $i++) {
            ...
            $profile = getFollowers($data_target['id'], $data_login, $req, $req, $proxy, $proxyauth);
            ...
            foreach ($profile as $rs) {
                ...
                $post = getPost($username, $data_login, $proxy, $proxyauth);
                if ($post['status'] == 'error') {
                    echo color()["LR"] . 'Error: ' . ucfirst($post['details']) . ' | ';
                    $data_follow = ($f == 'y' or 'y') ? followById($post['id'], $data_login, $proxy, $proxyauth) : '';
                    ...
                    $data_follow = ($f == 'y' or 'y') ? follow($username, $data_login, $proxy, $proxyauth) : '';
                    if ($data_follow['status'] == 'success') {
                        ...
                        $like = like($id_post, $data_login, $proxy, $proxyauth);
                        if ($like['status'] == 'error') {
                            ...
                            shuffle($x);
                            $text = $x[0];
                            $comment = comment($id_post, $data_login, $text, $proxy, $proxyauth);
                            ...
                            if ($comment['status'] == 'success') {
                                ...
                                $unfollow = unfollow($username, $data_login, $proxy, $proxyauth);
