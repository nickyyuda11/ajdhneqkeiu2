...
function findProfile($username, $session = 0, $proxy = 0, $proxyauth = 0)
{
    ...
    if ($session != 0) {
        ...
        if ($proxy) {
            $get = curl($url, 0, $header, 1, $proxy, $proxyauth);
        ...
        if ($proxy) {
            $get = curl($url, 0, 0, 1, $proxy, $proxyauth);
        } else {
            $get = curl($url);
        }
    }
...
function login($username, $password, $proxy = 0, $proxyauth = 0)
{
    ...
    // if ($proxy) {
    //     $get = curl($url_ig, 1, 1, 1, $proxy, $proxyauth);
    //     var_dump($get);
    // } else {
    $get = curl($url_ig);
    // }
    ...
    if ($proxy) {
        $ip = curl('https://api64.ipify.org/?format=json', 0, 0, 0, $proxy, $proxyauth);
        var_dump($ip);
        $post = curl($url_login, $body, $header, 1, $proxy, $proxyauth);
    } else {
...
function comment($id, $session, $text, $proxy = 0, $proxyauth = 0)
{
    ...
    if ($proxy) {
        $post = curlNoHeader($url, $body, $header, 1, $proxy, $proxyauth);
    } else {
...
function like($id, $session, $proxy = 0, $proxyauth = 0)
{
    if (isset($id)) {
        ...
        if ($proxy) {
            $post = curl($url, 1, $header, 1, $proxy, $proxyauth);
        } else {
...
function deletePost($id, $session, $proxy = 0, $proxyauth = 0)
{
    ...
    if ($proxy) {
        $delete = curl($url, 1, $header, 1, $proxy, $proxyauth);
    } else {
...
function getFollowers($username, $session, $count = 20, $pageSize = 20, $proxy = 0, $proxyauth = 0)
{
    ...
    while (true) {
        ...
        if ($url != '') {
            if ($proxy) {
                $response = curlNoHeader($url, 0, $header, 1, $proxy, $proxyauth);
            } else {
...
function getPost($username, $session = 0, $no = 0, $proxy = 0, $proxyauth = 0)
{
    ...
    if (!$session == 0) {
        ...
        if ($proxy) {
            $get = curl($url, 1, $header, 0, $proxy, $proxyauth);
        }
        $get = curl($url, 1, $header);
    } else {
